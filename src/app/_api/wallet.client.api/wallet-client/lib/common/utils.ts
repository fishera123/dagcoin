...
export class Utils {
  ...
  public static formatAmount(bytes, unit, opts): string {
    $.shouldBeNumber(bytes);
    $.checkArgument(_.includes(_.keys(Constants.UNITS), unit));

    function addSeparators(nStr, thousands, decimal, minDecimals): string {
      const amn = nStr.replace('.', decimal);
      const x = amn.split(decimal);
      let x0 = x[0];
      let x1 = x[1];
